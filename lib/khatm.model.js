...
class Khatm extends SqlTable{
  ...
  static pushNotification(){
    setInterval(() => {
      ...
      curSql[khatmTable].getNotRead()
        .then(res => {
          //Filter by deadline and remaining pages
          let data = Khatm.getNearDeadline(res);
              ...
              .then(res => {
                if(res){
                  for(let tk of res){
                    ...
                    request.post({
                      headers: {'Authorization': 'key=AAAAQiww4xI:APA91bFq0ohBvwPZecV3GvKs36pkW3zO2ZImTnJ5nZsduj84Ab8JAiaf7XB9S-6cpMBOP8O2IlTI6iBCmuJjy_v-ea5YNe2aSlM4gufMOOChuxQcRGjtuKSw2h5PwSiyGho9ZYEnhK5-',
  ...
  static getNearDeadline(data){
    let result = [];

    for(let item of data){
      let remainingDays = moment(item.end_date).diff(moment(new Date()), 'days');

      if(remainingDays > 0 && (remainingDays * 2) < item.remaining_pages)
        result.push({email: item.user_email,
