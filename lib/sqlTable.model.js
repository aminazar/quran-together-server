...
class SqlTable{
  constructor(tableName, idMember, test=false){
    ...
    this.curSql = test ? sql.test : sql;
    this.test = test;
    if(!this.curSql[tableName])
      throw new ReferenceError(`'${tableName}' is not a defiend SQL table`);
  }

  load(criteria) {
    return new Promise((resolve, reject) => {
      this.curSql[this.tableName].get(criteria)
        .then(data => {
          if(data.length>1)
  ...
  save() {
    return new Promise((resolve, reject)=>{
      ...
      let execute = this[this.idMember] ? //Updates in case the model contains an id, othewise adds data as new record
        //Update
        data => {
          this.curSql[this.tableName].update(data,this[this.idMember])
        ...
        //Insert
        data => {
          this.curSql[this.tableName].add(data)
            .then(idData=>{
