...
class Khatm extends SqlTable{
  ...
  static assigningPage(user_id, khatm_id, pages){
    ...
    return new Promise((resolve, reject) => {
      curSql[commitmentTable].getByUser({uid: user_id, khid: khatm_id})
        .then((res) => {
          let unread = [];
          res.forEach((el) => {
            if(!el.isread)
          ...
          if(pages > unread.length){
            //Add pages
            return this.addPages(user_id, khatm_id, pages - unread.length);
          }
            ...
            //Delete pages
            return this.deletePages(user_id, khatm_id, page_number_cids);
          }
          else
