        ...
        if(err)
          ...
          env.bcrypt.hash(Date.now().toString() + khatm_id.toString(), salt, null, (err, hash) => {
            if(err)
              ...
              this.sql[khatmTable].update({share_link: hash}, khatm_id)
  ...
  static assigningPage(user_id, khatm_id, pages){
    ...
    return new Promise((resolve, reject) => {
      curSql[commitmentTable].nonChosenPages({khid: khatm_id})
          ...
          if(res.length < pages)
            ...
            return db.tx(t => {
              ...
              queries = assigneePages.map((el) => {
                return t.query(env.pgp.helpers.update({uid: el.uid}, null, commitmentTable) + ' where cid =' + el.cid + ' returning page_number');
              });
        ...
        .then((res) => {
          let data = res.map(el => el[0].page_number);
          resolve(data);
        })
        .catch((err) => {
          console.log(err.message);
