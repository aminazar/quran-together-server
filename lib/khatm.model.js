...
class Khatm extends SqlTable {
  ...
  static getForceNotification(data) {
    ...
    for (let item of data) {
      ...
      if(remainingDays === 0 || ((item.remaining_pages / remainingDays) * 0.8 > item.read_pages && item.read_pages !== 0) || (item.remaining_pages > remainingDays * 5 && item.read_pages === 0))
        result.push({
          email: item.user_email,
          khatm_sharelink: item.share_link,
  ...
  static getFilteredTimezones(pivotTime) {
    ...
    for (let item of timezoneList) {
      ...
      //For better precision add time[1] === '00'
      //But for handling the delay is removed
      if (time[0] == pivotTime)
        result.push(time[3]);
    }
      ...
      curSql[everydayJoinTable].getEverydayUser({timezone: tz})
        .then(result => {
          for(let item of result){
            Khatm.assigningPage(item.uid, item.khid, item.page_per_day)
              .then(res => {
  ...
  static checkKhatmsToClear(){
    ...
    if(filteredTimezones.length > 0){
      curSql[khatmTable].clearDeadKhatmCommitments({
        end_date: moment().subtract(1, 'days').startOf('day').format('YYYY-MM-DD')
      })
        .then(res => {
