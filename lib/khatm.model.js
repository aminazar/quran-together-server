...
class Khatm extends SqlTable {
  ...
  static getFreePages(user_id, khatm_id){
    let curSql = Khatm.test ? sql.test : sql;

    return new Promise((resolve, reject) => {
      curSql[commitmentTable].getFreePagesWithUser({uid: user_id, khid: khatm_id})
        .then(res => resolve(res))
        .catch(err => reject(err));
    });
  }

  static selfAssigningPage(user_id, cids, shouldGet = false){
    let db = Khatm.test ? env.testDb : env.db;

    return new Promise((resolve, reject) => {
      if(shouldGet){
        let uniqueList = [];
        db.query('select * from commitments where cid in (' + cids.join(',') + ')')
          .then(res => {
            res.forEach(el => {
              if (el.uid === null)
                uniqueList.push(el.cid);
            });

            if(uniqueList.length > 0)
              return db.query('update commitments set uid =' + user_id + ' where cid in (' + uniqueList.join(',') + ')');
            else
              return Promise.reject('This page is assigned to another person. Please choose another page');
          })
          .then(res => {
            resolve(uniqueList);
          })
        ...
        db.query('update commitments set uid = null, isread = false where cid in (' + cids.join(',') + ')')
          .then(res => resolve(cids))
          .catch(err => reject(err));
      }
    });
  }
  ...
  static everydayHandler(user_id, khatm_id, shouldJoin){
    let curSql = Khatm.test ? sql.test : sql;
    let db = Khatm.test ? env.testDb : env.db;

    return new Promise((resolve, reject) => {
      if(shouldJoin){
        sql[everydayJoinTable].putData({khid: khatm_id, uid: user_id})
          .then(res => {
            return sql[khatmTable].getById({khid: khatm_id});
          })
          .then(res => {
            if(res.length > 0)
              return Khatm.assigningPage(user_id, res[0].khid, res[0].page_per_day, true);
            else
              reject('Khatm not found');
          })
          .then(res => resolve())
          .catch(err => {
            console.log(err);
        ...
        sql[everydayJoinTable].getRemains({uid: user_id, khid: khatm_id})
          .then(res => {
            if(res.length > 0)
              reject('You has unread committed pages');
            else
              return sql[everydayJoinTable].removeUser({khid: khatm_id, uid: user_id});
          })
          .then(res => resolve())
          .catch(err => reject(err));
      }
  ...
  static getForceNotification(data) {
    ...
    for (let item of data) {
      if(item.is_everyday){
        ...
        if(remainingDays >= 0 && (remainingDays !== 0 || parseInt(moment(new Date()).format('D')) !== parseInt(moment(item.end_date).format('D'))))
          remainingDays++;
        ...
        if((remainingDays === 0 && parseInt(item.remaining_pages) > 0) ||
          ...
          result.push({
            ...
            share_link: item.share_link,
            title: 'Quran Together Commitments',
            message: Khatm.notificationMessage(remainingDays, item.remaining_pages, item.khatm_name)
          });
      }
    }
  ...
  static deleteDeviceToken(token, email){
    return new Promise((resolve, reject) => {
      redis.get(email + '--device-token')
        .then(res => {
          if(res && res.length > 0){
            let tkList = res.filter(el => el !== token);
            if(tkList.length > 0)
              return redis.save(email + '--device-token', tkList);
            else
              return redis.remove(email + '--device-token');
          }
          else
            return redis.remove(email + '--device-token');
        })
        .then(res => resolve(JSON.stringify('Token-Device is removed')))
        .catch(err => {
          console.log(err);
          reject(err);
        });
  ...
  static sendingPushNotification(){
    let timezoneNearDeadline = Khatm.getFilteredTimezones('18');
    ...
    for (let tz of timezoneNearDeadline)
      curSql[khatmTable].getNotRead({timezone: tz})
        .then(res => {
          ...
          for (let item of data) {
            if(sentNotificationUsers.findIndex(u => u.email.toLowerCase() === item.email.toLowerCase() && u.share_link === item.share_link) !== -1){
              sentNotificationUsers = sentNotificationUsers.filter(u => u.email.toLowerCase() === item.email.toLowerCase() && u.share_link === item.share_link);
              continue;
            }
            else
      ...
      curSql[everydayJoinTable].getEverydayUser({timezone: tz})
        .then(result => {
          for(let item of result){
            if(insertEverydayPages.findIndex(p => p.uid === item.uid && p.khid === item.khid) !== -1){
              insertEverydayPages = insertEverydayPages.filter(p => p.uid === item.uid && p.khid === item.khid);
              continue;
            }
            else
              insertEverydayPages.push({
                uid: item.uid,
                khid: item.khid
              });
