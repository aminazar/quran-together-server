...
class Khatm extends SqlTable {
  ...
  static getForceNotification(data) {
    ...
    for (let item of data) {
      ...
      if ((item.remaining_pages / remainingDays) * 0.8 > item.read_pages || remainingDays === 0)
        result.push({
  ...
  static sendingPushNotification(){
    let timezoneNearDeadline = Khatm.getFilteredTimezones();
  ...
  static checkKhatmsToClear(){
    ...
    //Clear commitments table at 00:00 friday (every week)
    nodeSchedule.scheduleJob('0 0 * * 5', () => {
      curSql[khatmTable].clearDeadKhatmCommitments({
        end_date: moment().subtract(2, 'days').startOf('day').format('YYYY-MM-DD')
      })
        .then(res => {
